...
/// the eligible set. A retained per-item source-sequence watermark prevents an older full replay from
/// overwriting, deleting, or resurrecting newer eligibility state, whether or not the item itself is still
/// present. Ordering and
/// authorization are applied at query time by the pure <c>WhatsNextOrdering</c> and
/// <c>WhatsNextQueryAuthorization</c>, not stored here.
/// </summary>
public sealed class WorksWhatsNextTenantIndex
{
    ...
    /// <summary>
    /// The greatest accepted EventStore envelope position persisted for each work item, retained after the
    /// item leaves the eligible set. During additive rollout, an aggregate id with no entry here falls back to
    /// the sequence on any legacy eligible <see cref="Items"/> entry for that id. Once an entry exists here it
    /// is the sole ordering authority for that id: the item's own watermark comes from a different projection
