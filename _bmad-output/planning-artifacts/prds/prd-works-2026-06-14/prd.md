...
**Delivered-versus-target provenance (amended 2026-09-14).** The durable Epic 1–4 story map for historical Stories 1.1–4.9 records the delivered baseline. Requirements and acceptance details added after a story's delivery are target scope until a uniquely identified story implements them; they do not retroactively change historical acceptance evidence, story Status, story identity, or retrospective verdicts. In particular, this amended PRD is not evidence that later target requirements were acceptance criteria for, or were delivered by, Stories 1.1–4.9.

## 1. Vision

Hexalith.Works is the small, durable spine that owns a unit of *work to be done* and coordinates *who does it* — a system or AI agent, an internal user, or an external person reached by email — without becoming a task database or a workflow-diagram engine. A **Work Item** burns down toward zero remaining effort, competes for attention by priority and due date, spawns and suspends like a durable saga, and rolls its remaining effort up a parent→child tree so an objective's all-in remaining work is a single number when the tree shares a Unit — the default — and one number per Unit otherwise. Everything else—identities, dialogue, persistence, isolation, and IDs—is represented by late-resolved references to sibling Hexalith modules and is never copied.
...
- **UJ-2. Capability scenario — a system/AI executor burns down work through one uniform surface.** An authenticated service Party — channel = MCP, machine authority — is bound to a Work Item and reports progress in the item's Unit until Remaining reaches zero, at which point the item completes. The actor must match the bound Executor; the same commands and actor-to-binding rule let a human or external Party advance the Work Item, and the code path does not branch on executor kind. *Realizes the "everything is a Party / zero branching" build signal.*

- **UJ-3. A Work Item spawns a child, suspends, and resumes.** Ada's release-checklist item — driven by a system Party on the MCP channel — spawns a child for the sign-off it cannot do itself (through the Work-Tree Registry's reserve act — FR-16), parks itself on an Await-Condition (the child's completion), and the Reactor (FR-26) resumes it when the child completes, while the parent's rolled-up Remaining reflects the child's Burn-Down throughout. *Realizes the durable-saga + correct-Roll-Up build signal.* **Edge case:** if the item is also parked on a date that arrives first, the date trigger resumes it independently.
...
- **Burn-Down** — the trio of unit-tagged quantities **Estimated**, cumulative **Done**, and **Remaining** describing progress toward completion. `ReportProgress` advances cumulative Done by an accepted strictly positive delta; only the separately audited `CorrectProgress` act may replace or correct an already-recorded cumulative Done value. `ReEstimate` never changes Done. Progress is a fact (Remaining decreases), not a status flag. *Done* is always the quantity; the state is *Completed*. The v1 Burn-Down is the single Effort Meter; a second Cost Meter reuses the identical machinery later (Theme 5). *(Amended 2026-09-14.)*
...
- **Remaining** — for an estimated item, `max(Estimated - Done, 0)`, retained as the item's auditable Burn-Down quantity even after termination; it is undefined until the first estimate. **Completion invariant:** Remaining reaching 0 through progress completes the item; an explicit Complete may close an item whose reported Remaining is still above 0 (FR-8). Re-estimation alone never completes or reopens work. Roll-Up uses the distinct terminal-aware contribution defined by FR-11. *(Amended 2026-09-14; makes the derived formula and ReEstimate boundary explicit.)*
...
- **AuthorityLevel** — what an Executor Binding is permitted to do. Carried but not read in v1; value-based enforcement is Theme 4/6, while v1 still enforces actor-to-binding and trusted-origin relationships. *(Proposed ordered set — see §4.5 / `[ASSUMPTION]`.)*
...
- A caller-facing Create carrying a parent reference is rejected. A child Create carrying a parent reference is accepted only from the Reactor with evidence of the matching registry authorization in `Creating`; a `Reserved`, `Released`, superseded, or mismatched authorization produces the stable rejection defined by FR-16. *(Amended 2026-09-13 — closes the registry-first bypass.)*
...
- **Target delivery trace (amended 2026-09-14): Story 5.5, _Enforce the Obligation Bound Without Rewriting History_.** After Story 5.5 is delivered, the system enforces the 4,000-character maximum when admitting each new `CreateWorkItem` command. Historical `WorkItemCreated` payloads remain readable and replayable even when an Obligation accepted before Story 5.5 exceeds that limit; the new admission rule never rewrites or invalidates historical events.
...
- **The Unit is immutable once set** *(2026-09-08 product decision — was §13 OQ6, now AD-04)*: the first accepted estimate (at creation or by the first `ReEstimate`) establishes the item's Unit; every later `ReEstimate` preserves that Unit, and a later `ReportProgress` or `ReEstimate` in a different Unit is a domain rejection that leaves Unit, Estimated, Done, and Status unchanged.
- Estimated is never negative; for an estimated item, Remaining is derived as `max(Estimated - Done, 0)`. `Done > Estimated` is valid and remains visible as an overrun; only Remaining is clamped. Roll-Up uses FR-11's terminal-aware effective contribution rather than raw Remaining. *(Amended 2026-09-14.)*
...
A Work Item references at most one parent and zero-or-more children, and may hold one or more Await-Conditions while Suspended. The edge behind each reference is owned by the Work-Tree Registry; only durable child-creation evidence makes the edge `Attached` and authoritative, after which the parent records its child-reference bookkeeping. *(Amended 2026-09-13 — supersedes the 2026-09-08 ChildSpawned-first attachment rule.)*
...
Supplemental acts: `ReportProgress` is legal only from `InProgress` and may complete the item; `CorrectProgress` is legal from `InProgress` and from a `Completed` item whose completion was progress-driven, and may cross between those two states according to corrected Remaining, emitting `WorkItemReopened` when it reopens (FR-8). `Handoff` is legal from `InProgress` and `Suspended` and preserves Status (FR-17). `ReEstimate` and `Reschedule` are legal from every non-terminal state and preserve Status; `ReEstimate` also preserves cumulative Done and the established Unit and never completes or reopens work (FR-9). `LinkConversation` is lifecycle-neutral (FR-21). `SpawnChild` is legal as attachment bookkeeping under FR-16 and does not change the parent's Status, including after terminal closure. *(Amended 2026-09-14.)*

**Consequences (testable):**
- Only transitions in the table and the supplemental acts named above are accepted; any other transition is a domain rejection (`IRejectionEvent`), not an exception. State-based no-ops are limited to an exact duplicate terminal act, replay of the consumed Resume match, and explicitly named semantic equivalents; transport-key replay returns the original outcome (§10).
...
**Consequences (testable):**
- v1 `WorkItem` success-event catalog (additively extensible): `WorkItemCreated`, `WorkItemAssigned`, `WorkItemQueued`, `WorkItemClaimed`, `ProgressReported`, `ProgressCorrected`, `ReEstimated`, `WorkItemRescheduled`, `ChildSpawned`, `WorkItemSuspended`, `WorkItemResumed`, `WorkItemHandedOff`, `WorkItemCompleted`, `WorkItemReopened`, `WorkItemCancelled`, `WorkItemRejected`, `WorkItemExpired`, `ConversationLinked`. `ProgressCorrected`, `WorkItemReopened`, and `WorkItemHandedOff` are additive 2026-09-13 contracts; every existing payload remains unchanged. `WorkItemRescheduled` carries Priority and/or Due-Date changes (FR-9), and `ConversationLinked` realizes FR-21's post-creation reference link. *(Amended 2026-09-13; the former fifteen-event catalog was explicitly additive.)*
- The Work-Tree Registry's success-event catalog includes `EdgeReserved`, `ChildCreateAuthorized`, `EdgeAttached`, and `EdgeReleased`; `ChildCreateAuthorized` is the durable transition from `Reserved` to `Creating`, and only `Reserved` may advance to `Released` (FR-16). Dedicated audited rejection outcomes cover refused reservations and delayed, released, superseded, or mismatched saga legs without changing the existing `WorkItemTransitionRejected` shape. Registry contracts are governed like every other contract (catalog + golden corpus, §8). *(Amended 2026-09-14 — aligned with FR-16's `Reserved → Creating → Attached` sequence.)*
...
- **Progress correction is additive and auditable** *(added 2026-09-13)*: `CorrectProgress` records `ProgressCorrected` carrying the submitted corrected cumulative Done value and never deletes or rewrites history; prior values remain derivable from earlier events rather than copied into the Raw Act. `CorrectProgress` is restricted to the authenticated current Executor (§9), retains the established Unit, and accepts any corrected Done ≥ 0; Done may exceed Estimated, so the overrun remains visible and Remaining stays clamped at 0. From `InProgress`, corrected Remaining > 0 preserves `InProgress`, while corrected Remaining = 0 emits `WorkItemCompleted` and rests at `Completed`. From a progress-driven `Completed` item, corrected Remaining > 0 emits `ProgressCorrected` followed by `WorkItemReopened` and rests at `InProgress`; Remaining = 0 leaves it `Completed`. `CorrectProgress` is rejected for an explicitly completed or abnormally terminated item.
...
- `ReEstimated` changes only the absolute Estimated target: it preserves cumulative Done, the established Unit, and Status; derives `Remaining = max(Estimated - Done, 0)`; and never completes or reopens an item. `Done > Estimated` is a valid, visible overrun. `ReportProgress` may advance Done through a strictly positive delta, but only `CorrectProgress` may replace or correct the submitted cumulative Done value (FR-9). `ReEstimate` changes the plan, not the historical progress facts. *(Amended 2026-09-14.)*
...
**Consequences (testable):**
- `ReEstimated` records a new absolute Estimated target and is a normal, expected event — overrun and partial progress are native, not errors. Re-estimation never changes cumulative Done. For an estimated item it derives `Remaining = max(Estimated - Done, 0)`; it preserves the established Unit and Status and never completes or reopens the item. `Done > Estimated` is valid and remains visible as the overrun; Done is not clamped. `ReportProgress` advances cumulative Done only through an accepted strictly positive delta, while only the separately audited `CorrectProgress` act may replace or correct the submitted cumulative Done value. The first accepted estimate on an unestimated item establishes its Unit (FR-3). *(Amended 2026-09-14.)*
...
**Description:** Suspension-on-an-event is the primitive: a Work Item parks on an Await-Condition and resumes when the matching event arrives. v1 defines the Resume command contract and handles child-completion and date/timer natively; only the external signal adapters are deferred. Realizes UJ-3.
...
A Work Item can gain one or more children, optionally suspending itself awaiting them. The **public act is the Work-Tree Registry's reserve command**, which carries the complete child-creation payload verbatim (Obligation, optional Estimated/Unit, Schedule, Executor Binding, Conversation reference, and whether the parent suspends awaiting the child). The Registry authorizes creation, the Reactor submits the origin-restricted child Create (FR-1), durable child-creation evidence makes the edge `Attached`, and only then does the Reactor submit the parent's bookkeeping-only `SpawnChild`. `SpawnChild` is not builder-facing and never makes the edge authoritative. *(Amended 2026-09-13 — aligned with the latest AD-21 sequence.)*

**Consequences (testable):**
- A builder or Executor requests a child by issuing the registry reserve command; a direct child Create carrying a parent or direct `SpawnChild` from an ordinary tenant-member identity is denied (§9 trusted origin).
- The observable registry lifecycle is `Reserved → Creating → Attached` or `Reserved → Released`. The Registry may release only from `Reserved`, before it authorizes creation. `Creating` cannot be released because of a timeout. It remains durably recoverable until either matching child-creation evidence attaches it or an audited repair proves a permanent conflict and records the orphan disposition. Timeout policy is tenant-specific Platform configuration; its binding budget is gate G8.
- Matching durable `WorkItemCreated` evidence is the sole basis for `Attached`. `Attached` then drives the parent's idempotent `SpawnChild` bookkeeping and `ChildSpawned`; this bookkeeping is accepted even if the parent became terminal meanwhile, without changing terminal Status. When that parent is `Cancelled` or `Expired`, processing must durably schedule catch-up termination for the newly attached child before acknowledging the bookkeeping outcome.
...
The **Reactor** performs every behavior that spans more than one aggregate: the FR-16 child-creation sequence, resuming a parent when its child completes (FR-15), cascading termination through a subtree (FR-10), and issuing date or expiry reminders (FR-10, FR-15). It is a mechanical process manager that translates Domain Events into commands and lives outside the `WorkItem` and Work-Tree Registry aggregates. The aggregates' `Handle` stays pure and single-aggregate (§9); the Reactor is the one place a cross-aggregate step may be taken. *(Amended 2026-09-14 — aligns the summary with FR-16's adopted sequence.)*
...
**Consequences (testable):**
- A `Queued` item can be claimed by an authenticated tenant member only on their own behalf, emitting `WorkItemClaimed` and transitioning the item to `InProgress` bound to that claimant. The same `Claim` act is how the authenticated bound Executor starts an `Assigned` item (FR-6) — one entry act into `InProgress` for both modes.
- **Single claim wins:** the aggregate's single-writer/optimistic-concurrency model serializes competing claims: exactly one succeeds, and the loser receives a domain rejection because the item is no longer claimable (mechanism detailed at architecture; see §9).
- An `Assigned` item can be returned to `Queued` (requeue) and a `Queued` item can be directly assigned — both normal transitions; requeue re-emits `WorkItemQueued` (which thus marks every entry into the queue, whether from `Created` or `Assigned`). `[ASSUMPTION: claim eligibility scoring is a routing concern and is deferred to Theme 4; v1 allows an authenticated member of the tenant to claim a Queued item only on their own behalf.]`
...
- Both ports are deliberately present in v1, although `IExecutorRouter` has no implementation and `IExpectationResolver` has only a literal implementation: a C# port with no v1 caller has no event-schema cost, and these two are the *named* seams SM-C2 exempts from its "speculative machinery" rule — anything beyond them is not. *(Rationale added 2026-09-08.)*
...
| Claim a `Queued` item | An authenticated tenant member claiming only on their own behalf; the resulting binding PartyId is the trusted acting Party. |
...
| **G8** | Binding performance/recovery budgets — Product + Architecture | Environment, dataset, percentile, measurement window, failure policy, retry/redelivery horizon, coordination deadlines, recovery objectives, and Registry edge-cardinality/attach-throughput/rehydration envelopes. | Distributed acceptance |
...
**Roadmap open item — no-login action envelope.** Product + Security must classify which external-email actions may retain the one-tap/no-login promise under a bound, expiring, single-use credential and which risk classes require step-up authentication. Resolve this before Theme 3 email acceptance so downstream UX neither weakens authority nor silently breaks the promised journey.
...
*This index contains every current `[ASSUMPTION]` and the material superseded assumptions retained for audit. The June entries were confirmed by the user on 2026-06-14; later entries record what validation or architecture replaced. Current tags stay in place so later phases can challenge any that prove unworkable.*
...
- §4.1 FR-2 — *(2026-09-08; delivery trace amended 2026-09-14)* Obligation description bounded at 4,000 characters; longer content lives in a linked Conversation or referenced document. Story 5.5 delivers the target admission enforcement without invalidating or rewriting longer historical payloads.
...
- §4.2 FR-7 — The former fifteen-event v1 `WorkItem` catalog was always additive. **Superseded 2026-09-13:** `ProgressCorrected`, `WorkItemReopened`, and `WorkItemHandedOff` bring the current success catalog to eighteen without changing existing payloads; registry events remain additional contracts.
...
- §4.5 FR-18 — Claim eligibility scoring is deferred to Theme 4; in v1 an authenticated tenant member may claim a `Queued` item only on their own behalf, while an `Assigned` item may be claimed only by its bound Executor (single claim wins). *(Relationship enforcement amended 2026-09-13.)*
