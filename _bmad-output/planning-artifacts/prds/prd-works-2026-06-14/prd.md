...
- **Progress correction is additive and auditable** *(added 2026-09-13)*: `CorrectProgress` records `ProgressCorrected` carrying the submitted corrected cumulative Done value and never deletes or rewrites history; prior values remain derivable from earlier events rather than copied into the Raw Act. It is responsibility-bound to the authenticated current Executor (§9), retains the established Unit, and accepts any corrected Done ≥ 0; Done may exceed Estimated so over-run remains visible and Remaining stays clamped at 0. From `InProgress`, corrected Remaining > 0 preserves `InProgress`, while corrected Remaining = 0 emits `WorkItemCompleted` and rests at `Completed`. From a progress-driven `Completed` item, corrected Remaining > 0 emits `ProgressCorrected` followed by `WorkItemReopened` and rests at `InProgress`; Remaining = 0 leaves it `Completed`. It is rejected for an explicitly completed or abnormally terminated item.
...
- The observable registry lifecycle is `Reserved → Creating → Attached` or `Reserved → Released`. The Registry may release only from `Reserved`, before it authorizes creation. `Creating` cannot timeout-release: it remains durably recoverable until matching child-creation evidence attaches it or audited repair proves a permanent conflict and marks the orphan disposition. Timeout policy is tenant-specific Platform configuration; its binding budget is gate G8.
