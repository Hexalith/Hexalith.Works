...
- **Remaining** — Estimated minus Done for this item (never below 0); the quantity that rolls up. **Completion invariant:** Remaining reaching 0 through progress completes the item, and a `Completed` item always contributes 0 Remaining; an explicit Complete may close an item whose reported Remaining is still above 0 (FR-8). *(Amended 2026-09-08; was "Done = Remaining is 0 is the completion invariant".)*
...
- `ReEstimated` never completes an item: a re-estimate below Done clamps Remaining to 0 and leaves the Status unchanged; Done exceeding Estimated is the visible record of an over-run (FR-9).
- Completion is terminal in v1 — there is no Reopen. An Executor who reports the full remaining effort before the work is truly finished is completed by Burn-Down and cannot re-estimate; report progress in increments, or `ReEstimate` upward first. `[NOTE FOR PM: a `Reopen` act (Completed → InProgress) is deferred; revisit if Theme 3 channel surfaces show early completions in practice. Owner: PM.]`
...
**Consequences (testable):**
- For any Work Item, rolled Remaining = own Remaining + Σ(own Remaining of every descendant reachable through `Attached` edges) — the same total as the recursive "own + children's rolled" reading, stated flat so each descendant is counted once from its own stream.
...
- A descendant's `ProgressReported` / `ReEstimated` / `WorkItemCompleted` / terminal event is reflected incrementally in every ancestor's rolled Remaining (no whole-stream re-read per query).
...
- An **unestimated** Work Item contributes 0 Remaining and is counted in an *unestimated descendants* count exposed alongside the rolled figures, so a rolled 0 is distinguishable from "nothing estimated yet". `[ASSUMPTION — added 2026-09-08; additive read-model field.]`
...
- Changing Channel for the *same* Party (e.g., a colleague moving from email to chatbot) is a reassign that emits `WorkItemAssigned` with the new binding, under the same state rules as any reassign; there is no separate channel-change act. *(Added 2026-09-08 — see SM-5.)*
...
- **v1 blast radius, stated plainly** *(2026-09-08)*: the only gate on any act in v1 is the §9 baseline — the caller is an authenticated member of the Work Item's Tenant. AuthorityLevel is carried, not read. Consequently *any* tenant member may assign, reassign, claim, re-estimate, reschedule, cancel, or expire *any* Work Item of the tenant, and a cancel or expire cascades to the whole open subtree unguarded (FR-10). No v1 behaviour branches on AuthorityLevel. `[NOTE FOR PM: this is acceptable for a kernel exercised by builders in a platform-hosted harness, and not for a shared tenant. Revisit — and decide whether Coordinate/Administer gating on Cancel/Expire/Assign lands before Theme 4 — before the first multi-team tenant is admitted to production or at Theme 4 planning, whichever comes first. Owner: PM.]`
...
**Consequences (testable):**
- **Two views, one query** *(2026-09-08 product decision)*: called with an Executor's `PartyId`, the query returns that Party's `Assigned` items plus the tenant's `Queued` pool ("what should *I* do next — my pushed work and what I could claim"); called without one, it returns every `Assigned` and `Queued` item of the tenant (the coordinator's view). The filter predicate is exactly: tenant membership of the caller (§9) → Status ∈ {`Assigned`, `Queued`} → for the Executor view, binding `PartyId` equals the given Party *or* Status is `Queued`. No eligibility scoring, ranking, or routing input takes part (Theme 4).
...
- v1 builds **no** production channel adapters (email/magic-link, chatbot, MCP, CLI), **no** LLM-native interaction or NL parsing, **no** executor routing/escalation engine, **no** cost meter/spend governance, and **no Theme 6 hardening** (step-up auth, signed single-use links, consent/residency routing, DoS guards). These are the Theme 2 remainder and Themes 3–6 (§12). The platform-owned identity-provenance and trusted-origin **baseline** (authenticated callers, tenant derived from verified claims, deny-before-dispatch, mTLS) is *not* Theme 6 and *is* v1 — see §9. *(Narrowed 2026-09-08 — AD-23/AD-24; was "no security-hardening enforcement".)* *(The brainstorm filed MCP-as-actor-channel and CLI-as-scriptable-work under foundation Theme 2; v1 deliberately defers even these non-LLM command surfaces — the kernel-only surface decision — while keeping the Channel seam so they attach later without core changes. They are the "Theme 2 remainder" row in §12.)*
...
- **Performance (qualitative for v1).** The Roll-Up and "what's next" projections remain responsive for realistically deep/wide trees by updating incrementally (FR-11), without re-reading whole streams on each query. No numeric targets are pinned in v1 (acceptance is build-signal based — §11). `[ASSUMPTION: numeric performance budgets deferred to a later iteration.]` `[ASSUMPTION — indicative v1 fixture, added 2026-09-08: a Work Tree of depth 32 with top-level fan-out 50 (≈1,600 items) under the platform lane; the Roll-Up converges within the SM-2 bound after the last event is published, and a rolled read answers in < 200 ms from the read model. Provisional until budgets are pinned.]`
...
- **SM-1 — Full event-sourced lifecycle, durable across restart.** The sequence create → progress → spawn child → suspend-on-event → resume → complete runs end-to-end under the platform-owned Aspire topology; and a Work Item suspended mid-saga rehydrates from its event stream after a restart and resumes correctly (durability is a fact, not a claim). The same run covers the **claim race** (two Executors, one `Queued` item — exactly one `WorkItemClaimed`, one domain rejection; FR-18) and a **cascade** (a parent cancelled with an open subtree — every active descendant terminated, terminal ones untouched; FR-10). *Validates FR-1, FR-6–FR-8, FR-10, FR-14–FR-16, FR-18, FR-24, FR-26.* *(Scenarios extended 2026-09-08.)*
- **SM-2 — Correct Roll-Up (at quiescence).** For any constructed Work Tree, rolled Remaining equals own + descendants' Remaining, and updates as descendants progress. The equality is asserted at **quiescence**: once every published event of the tree has been acknowledged by the Roll-Up projection (tests await acknowledgement rather than sleeping); under the platform-hosted topology, convergence is expected within a configured bound. `[ASSUMPTION: SM-2 convergence bound = 5 s for the SM-2 fixture under the platform lane; provisional, tightened when §9 performance budgets are pinned.]` *Validates FR-11–FR-13.* *(Amended 2026-09-08.)*
...
**Secondary**
- **SM-5 — Channel change = one operation.** A Party changing Channel mid-work (email → chatbot, MCP → CLI) is a single reassign emitting one `WorkItemAssigned`, with no channel-specific act and no change to the item's Status or Burn-Down, demonstrated by test. *Validates FR-17.* *(Repointed 2026-09-08; was a duplicate of SM-3's human⇄AI handoff.)*
- **SM-6 — Rebuild from zero reproduces the read models.** Replaying every tenant stream into fresh Roll-Up and "what's next" projections yields read models identical to the live ones at quiescence, and no event payload in the golden corpus carries a derived value (rolled totals, ordering positions, interpreted Expectations). *Validates FR-7, FR-11, FR-20, §9 rebuildability — the "AI never in the system of record" bet.* *(Added 2026-09-08.)*
...
- §4.3 FR-11 — *(2026-09-08)* An unestimated Work Item contributes 0 Remaining and is counted in an *unestimated descendants* count (additive read-model field).
...
- §9 NFRs — *(2026-09-08)* Indicative v1 fixture: depth 32 × top-level fan-out 50 (≈1,600 items); Roll-Up converges within the SM-2 bound; rolled read < 200 ms. Provisional.
- §11 SM-2 — *(2026-09-08)* SM-2 convergence bound = 5 s for the fixture under the platform lane. Provisional.
