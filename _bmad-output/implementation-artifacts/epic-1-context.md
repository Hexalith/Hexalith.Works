...
- Public contracts evolve additively and remain serialization-tolerant: no `V2` event types and no breaking previously persisted payloads. Register commands and events with `Hexalith.PolymorphicSerializations` and protect compatibility with golden-payload contract tests.

## Technical Decisions

- Use the Hexalith domain-module layout and central build configuration: `.slnx`, central package versions, warnings as errors, .NET SDK `10.0.301` with latest-patch roll-forward, and focused xUnit v3 tests. Project files must not carry inline package versions.
...
- Enforce dependency direction `Contracts ← Server ← Projections`; testing and the minimal host reference inward, while `Reactor` references Contracts only. `Contracts` stays low-dependency, and the kernel references no adapter or infrastructure implementation.
...
- State-changing Works events carry payload `(AggregateId, Sequence)`, where `Sequence` is the state-changing ordinal. EventStore envelope `SequenceNumber` remains the persisted stream and delivery position. Rejection payloads keep their context-only shape and apply as state no-ops.
