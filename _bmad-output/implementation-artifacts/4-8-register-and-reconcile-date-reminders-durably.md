...
- **AC #1 status (honest):** suspend-time registration is implemented and deterministically proven; the live resume-without-restart depends on the Dapr actor-reminder fire (Story-4.6 infra), which the WSL2 `dapr init` sandbox does not deliver. Recorded as a substrate blocker (test-summary), not hidden. AC #2/#3 proven live in prior review rounds; AC #4 proven by unchanged green kernel-purity guards. **This session's own Tier-3 attempt (2026-09-05) did not add a fresh live-pass or live-fail data point** — the AppHost `DistributedApplication.StartAsync` hang (finding line 118, still open) reproduced again and the run was terminated before its diagnostic output flushed (see Debug Log). AC #1 and the AppHost-startup finding remain open exactly as the prior review rounds left them.
