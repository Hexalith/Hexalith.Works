...
|---------|---------|-------------------|
| 4603 `RecoveryStepFailed` | A recovery operation identified by the structured `Reason` did not complete, and the original cause is attached. `startup-reminder-reconciliation` identifies a bounded startup retry loop; `startup-cascade-recovery` identifies one startup pass; `startup-cascade-replay`, discovery/read, and unregister reasons identify narrower operations and do not alone prove a whole startup attempt failed. | Filter by `Reason`, inspect the protected cause and adjacent subsystem warnings, and check the named state-store, EventStore, Dapr, or actor dependency. For `startup-reminder-reconciliation`, a later same-`Reason` 4603 proves another failed attempt; absence of one can mean success, host shutdown, or retry-budget exhaustion, so inspect host lifecycle, configured attempt count, and concrete progress. `startup-cascade-recovery` promises no in-process repeat; restart Works after dependency repair if that pass must run again. Verify reminder/cascade/actor progress and that same-`Reason` warnings stop; there is no universal recovery-success event. |
...
| 4605 `PendingDateAwaitScanIncomplete` | One or more tenant/index, parking, or candidate-stream reads failed. Partial results are attempted after this warning; a later non-cancellation processing failure produces 4603, and scheduling failure also emits 4609. Exact caller cancellation ends quietly without 4603. The warning includes failed-tenant, failed-candidate, and parked-skip counts. | Check the attached cause and applicable 4604/4606/4608 row, restore the failing state-store or EventStore dependency, and correlate any later 4603 by structured `Reason`. Watch for repeated warnings after the configured delay and concrete 4600–4602 recovery activity. If warnings stop, distinguish successful progress, host shutdown, and retry-budget exhaustion using host lifecycle and the configured attempt count before restarting Works. |
...
still fire, but recovery cannot repair a missing registration while the aggregate remains parked. A 4603, 4604,
4605, 4606, 4608, or 4609 is dependency/failure evidence; during startup recovery it is handled by the bounded
retry policy. Never delete parking or reminder state
